...
struct __tuple_constraints
{
  ...
  __check_tuple_like_constructible(__tuple_indices<_Indices...>) noexcept
  {
    ...
    { // [tuple#cnstr]-21.3: either sizeof...(Types) is not 1
      // [tuple#cnstr]-29.5: either sizeof...(Types) is not 1
      if constexpr (__is_cuda_std_tuple<remove_cvref_t<_UTuple>>
                    && (is_same_v<_Types, tuple_element_t<_Indices, remove_cvref_t<_UTuple>>> && ...))
      { // [tuple#cnstr]-21.3: is_same_v<T, U> is false
        return _InvalidTupleConstructor{};
      }
      else if constexpr ((is_convertible_v<_UTuple, __type_index_c<0, _Types...>>
                          || is_constructible_v<__type_index_c<0, _Types...>, _UTuple>) )
      { // [tuple#cnstr]-21.3: is_convertible_v<_UTuple, T>, is_constructible_v<T, _UTuple> are false
        // [tuple#cnstr]-29.5: is_convertible_v<_UTuple, T>, is_constructible_v<T, _UTuple> are false
        return _InvalidTupleConstructor{};
      }
      else
